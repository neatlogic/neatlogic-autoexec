...
public class AutoexecJobSourceTypeHandler extends AutoexecJobSourceTypeHandlerBase {
    ...
    @Override
    protected int getJobPhaseSqlCount(AutoexecJobPhaseNodeVo jobPhaseNodeVo) {
        return autoexecJobMapper.searchJobPhaseSqlCount(jobPhaseNodeVo);
    }

    @Override
    protected List<ISqlDetail> searchJobPhaseSqlForExpport(AutoexecJobPhaseNodeVo jobPhaseNodeVo) {
        List<ISqlDetail> result = new ArrayList<>();
        List<AutoexecSqlDetailVo> list = autoexecJobMapper.searchJobPhaseSql(jobPhaseNodeVo);
        if (list.size() > 0) {
